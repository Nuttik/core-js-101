...
function isTriangle(a, b, c) {
  /**
   * Треугольник можно построить по трём сторонам,
   * если длина большой стороны меньше чем сумма длин двух других сторон.
   */
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const result = `${start + min}, ${max}${end}`;
  return result;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  if (resulte.includes('/') && resulte.length > 1) {
    resulte = resulte.slice(0, resulte.lastIndexOf('/') + 1);
  }
  return resulte;
}
